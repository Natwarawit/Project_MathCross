...
func check_horizontal() -> bool:
	var checks = [
		[[0, 2, 4, 28], [2,0,2,13],[4,0,2,30],[6,0,2,17]],  # กริดที่ 1
		[[0,1,3,17],[3,0,2,13],[6,1,3,17]], # กริดที่ 2
		[[0,0,2,21],[4,2,4,8]], # กริดที่ 3
		[[0,0,2,32],[3,1,3,11],[6,2,4,29]] , # กริดที่ 4
		[[1,0,2,24],[3,2,4,27]] , # กริดที่ 5
	]
...
func check_vertical() -> bool:
	var checks = [
		[[0, 2, 4, 12],[4,0,2,30] ],  # กริดที่ 1
		[[1,2,4,9],[3,0,2,13]], # กริดที่ 2
		[[0,0,2,26],[2,2,4,4],[4,2,4,21]], # กริดที่ 3
		[[2,2,4,8],[4,0,2,12]],# กริดที่ 4
		[[0,1,3,2],[3,2,4,15]],# กริดที่ 4
	]
	for check in checks[current_grid_index]:
		if len(check) < 4:
			continue  # ถ้า check มีขนาดไม่พอให้ข้าม
...
func start_timer():
	...
	timerlabel.text = "Time Remain : " + str(time_remaining)  # แสดงเวลาที่เหลือใน label
	...
	while time_remaining > 0:  # เช็คว่าเวลายังเหลืออยู่
		...
		if timer_running:  # ตรวจสอบว่าสถานะของ timer_running เป็นจริง
			...
			timerlabel.text = "Time Remain : " + str(time_remaining)  # อัปเดตข้อความใน Label
...
func reset_timer():
	time_remaining = 60  # เริ่มต้นเวลาที่ 60 วินาที
	timerlabel.text = "Time Remain : " + str(time_remaining)  # แสดงเวลาที่เหลือ
...
func _on_back_button_pressed():
	if used_grid_indexes.size() == grid.size():
		...
		get_tree().change_scene_to_file("res://tscn/EndEasyStory.tscn")  # Change to the story scene
	else:
