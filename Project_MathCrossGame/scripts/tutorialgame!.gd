...
@onready var level  = $"Level State"
var is_victory: bool = false
var current_level: int = 1  # เก็บว่ากำลังเล่นเลเวลไหน (เซ็ตตอนเข้าเลเวล)
...
func show_victory_message():
	is_victory = true  # ผู้เล่นชนะ
	# ปลดล็อกด่านถัดไป
	level.unlock_next_level(current_level)

	# แสดงข้อความ
	var victory_label = Label.new()
	victory_label.text = "ยินดีด้วยคุณชนะแล้ว!"
	victory_label.modulate = Color(0, 1, 0)  # เขียว
	add_child(victory_label)

	# รอ 3 วินาทีแล้วลบ label
