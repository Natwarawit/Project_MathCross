...
func _update_mission_ui():
	...
	if not gate_unlocked:
		...
		_set_gray(incorrect_label, "ตอบผิด ≤ 4 (รอเคลียร์ด่าน)")
	...
	if incorrect_count <= 4:
		_set_green(incorrect_label, "ตอบผิด: %d / 4 ✔" % incorrect_count)
	else:
		_set_red(incorrect_label, "ตอบผิดเกิน 4 ครั้ง ✘")
...
func _can_enter_portal() -> bool:
	...
	return (
		math_cleared
		and player.health > global.max_health * 0.5
		and incorrect_count <= 4
		and eat_count < 3
	)
