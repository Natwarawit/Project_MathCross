...
func _ready():
	...
	if has_node("player"):
		...
		$player.died.connect(_on_player_died)   # ✅ ฟังตอน player ตาย
...
func _update_mission_ui():
	...
	if not gate_unlocked:
		_set_gray(hp_label, "HP > 75% (รอเคลียร์ด่าน)")
		_set_gray(incorrect_label, "ตอบผิด ≤ 5 (รอเคลียร์ด่าน)")
		_set_gray(regen_label, "ฟื้นฟู < 3 (รอเคลียร์ด่าน)")
		return

	# ---------- HP ----------
	if player.health > global.max_health * 0.75:
		_set_green(hp_label, "HP > 75% ✔")
	else:
		_set_red(hp_label, "HP ต้องมากกว่า 75% ✘")

	# ---------- Incorrect ----------
	if incorrect_count <= 5:
		_set_green(incorrect_label, "ตอบผิด: %d / 5 ✔" % incorrect_count)
	else:
		_set_red(incorrect_label, "ตอบผิดเกิน 5 ครั้ง ✘")
